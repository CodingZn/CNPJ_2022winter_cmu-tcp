 ...
 * 
 * 实现window后，接收端只按顺序确认收到的pkt, 待调试 --xtl
 *
...
void handle_message(cmu_socket_t *sock, uint8_t *pkt) {printf("handle message\n");
  ...
  switch (flags) {
    case SYN_FLAG_MASK:{// listener
      ...
      uint16_t hlen = sizeof(cmu_tcp_header_t) + TIMESTAMP_OPTION_SIZE;
    ...
    case ACK_FLAG_MASK: {// 发送端接受ack,滑动窗口, 获取rtt
      uint32_t ack = get_ack(hdr);                        printf("rcv ack\n");  
      if (after(ack, sock->window.last_ack_received)) {// 窗口的base 就是window.last_ack_received
    ...
    default: {// recv收到数据，回ack, for debug with a window
      socklen_t conn_len = sizeof(sock->conn);         printf("rcv data\n");  
      uint32_t seq = sock->window.last_ack_received;

      uint32_t rcvd_seq = get_seq(hdr);
      ...
      // 仅确认按序到达的pkt
      uint32_t ack;
      if (rcvd_seq == sock->window.next_seq_expected){
        ack = rcvd_seq + get_payload_len(pkt);
      }
      else{
      ...
      // 仅接受按序到达的pkt
      if (rcvd_seq == sock->window.next_seq_expected) {
        sock->window.next_seq_expected = rcvd_seq + get_payload_len(pkt);
 ...
 * 
 * 在socket中增加了timeout的指示符以指示一次checkdata是否超时 --xtl
 * RTO 待完善
 *
 ...
 */ /*
void single_send(cmu_socket_t *sock, uint8_t *data, int buf_len) {printf("single send: buflen = %d", buf_len);
  uint8_t *msg;
